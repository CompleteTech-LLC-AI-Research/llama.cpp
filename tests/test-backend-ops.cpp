...
// Test SET_ROWS with turbo3 destination.
// This validates the full quantization pipeline: f32 -> WHT -> PolarQuant -> turbo3
// followed by the test harness dequantizing both backend outputs for comparison.
...
struct test_set_rows_turbo3 : public test_case {
    ...
    void initialize_tensors(ggml_context * ctx) override {
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
            ...
            } else if (t->type == GGML_TYPE_TURBO3_0) {
...
// Test SET_ROWS with turbo4 destination.
// This mirrors the Gemma4 V-cache path used by --cache-type-v turbo4.
