...
int ggml_metal_op_turbo_wht(ggml_metal_op_t ctx, int idx) {
    ...
    int group_size;
    memcpy(&group_size, (const char *) op->op_params + sizeof(int), sizeof(int));
    if (group_size == 0) {
        group_size = 128;
    }
    ...
    ggml_metal_encoder_set_bytes   (enc, &args, sizeof(args), ida++);
    ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[0]), ida++);
    ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[1] != nullptr ? op->src[1] : op->src[0]), ida++);
    ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op),         ida++);
