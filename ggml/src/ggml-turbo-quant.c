...
GGML_API void turbo_cpu_set_wht_group_size(int group_size);
...
/* CPU SET_ROWS quantization runs per worker thread. Keep the selected WHT
 * group size thread-local so concurrent contexts cannot race through shared
 * process state. */
static TURBO_THREAD_LOCAL int turbo_cpu_wht_group_size = 0;
...
static double turbo_prng_normal(uint64_t * state) {
...
static void turbo_init_rotation_impl(void) {
    ...
    uint64_t prng_state = TURBO_SEED_ROTATION;
    float G[TURBO_D * TURBO_D];
    for (int i = 0; i < d * d; i++) {
        G[i] = (float)turbo_prng_normal(&prng_state);
    }
...
#if defined(_WIN32)
static BOOL CALLBACK turbo_init_rotation_once(PINIT_ONCE init_once, PVOID parameter, PVOID * context) {
...
static void turbo_init_rotation(void) {
    ...
    InitOnceExecuteOnce(&once, turbo_init_rotation_once, NULL, NULL);
#else
    static pthread_once_t once = PTHREAD_ONCE_INIT;
    pthread_once(&once, turbo_init_rotation_impl);
#endif
}
...
static void turbo_init_qjl_impl(void) {
    const int d = TURBO_D;
    uint64_t prng_state = TURBO_SEED_QJL;

    for (int i = 0; i < d * d; i++) {
        turbo_qjl_matrix[i] = (float)turbo_prng_normal(&prng_state);
    }
...
#if defined(_WIN32)
static BOOL CALLBACK turbo_init_qjl_once(PINIT_ONCE init_once, PVOID parameter, PVOID * context) {
    (void)init_once; (void)parameter; (void)context;
    turbo_init_qjl_impl();
    return TRUE;
}
...
#if defined(_WIN32)
    static INIT_ONCE once = INIT_ONCE_STATIC_INIT;
    InitOnceExecuteOnce(&once, turbo_init_qjl_once, NULL, NULL);
#else
    static pthread_once_t once = PTHREAD_ONCE_INIT;
    pthread_once(&once, turbo_init_qjl_impl);
#endif
}
...
void quantize_row_turbo3_0_ref(const float * GGML_RESTRICT x, block_turbo3_0 * GGML_RESTRICT y, int64_t k) {
    ...
    // Read WHT group size from thread-local SET_ROWS state.
