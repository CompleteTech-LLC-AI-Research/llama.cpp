...
TURBO_IQ_IMPORT uint64_t turbo_innerq_generation(void);
TURBO_IQ_IMPORT bool turbo_innerq_scale_inv_snapshot(float * dst, uint64_t * generation);
...
[[maybe_unused]] static uint64_t turbo_innerq_generation(void) { return 0; }
[[maybe_unused]] static bool turbo_innerq_scale_inv_snapshot(float * dst, uint64_t * generation) { (void)dst; (void)generation; return false; }
...
llama_kv_cache::llama_kv_cache(
    ...
    for (uint32_t il = 0; il < hparams.n_layer; il++) {
        ...
        if (turbo_rotation == nullptr && (k_is_turbo || v_is_turbo)) {
...
ggml_cgraph * llama_kv_cache::build_graph_shift(llm_graph_result * res, llama_context * lctx) const {
    ...
    for (const auto & layer : layers) {
        ...
        const auto n_rot         = hparams.n_rot(il);
        const auto n_embd_head_k = n_embd_k_gqa / n_head_kv;
...
bool llama_kv_cache_context::apply() {
    ...
    if (kv->get_turbo_innerq_scale_inv() != nullptr && turbo_innerq_needs_tensor_update()) {
        if (kv->update_turbo_innerq_scale_inv(turbo_innerq_generation())) {
...
bool llama_kv_cache::update_turbo_innerq_scale_inv(uint64_t generation) {
    ggml_tensor * t = get_turbo_innerq_scale_inv();
    if (t == nullptr || t->buffer == nullptr || generation == 0 || turbo_innerq_scale_generation == generation) {
        return false;
    }

    float scale_inv[INNERQ_MAX_CHANNELS];
    uint64_t published_generation = 0;
    if (!turbo_innerq_scale_inv_snapshot(scale_inv, &published_generation) ||
        published_generation == 0 ||
        turbo_innerq_scale_generation == published_generation) {
        return false;
    }

    ggml_backend_tensor_set(t, scale_inv, 0, INNERQ_MAX_CHANNELS * sizeof(float));
    turbo_innerq_scale_generation = published_generation;
    return true;
}
